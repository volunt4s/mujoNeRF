...
class CameraSet:
    def __init__(self, xml_string, y_target_angle, y_times, z_target_angle, z_times):
        self.ref_pos, self.ref_xyaxes = self.xml_string_parsing(xml_string)
        self.camera_id_lst = []
        self.camera_xml_lst = self.get_hemisphere_camera_xml(y_target_angle, y_times, z_target_angle, z_times)

    def xml_string_parsing(self, xml_string):
        root = ET.fromstring(xml_string)
        pos_attr = root.attrib.get('pos')
        xyaxes_attr = root.attrib.get('xyaxes')
        pos_lst = [float(x) for x in pos_attr.split()]
        xyaxes_lst = [float(x) for x in xyaxes_attr.split()]
        pos_np = np.array(pos_lst)
        xyaxes_np = np.array(xyaxes_lst)

        return pos_np, xyaxes_np
